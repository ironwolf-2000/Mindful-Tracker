...
function convertToPercentage(
  ...
  // Use habit's goal if available, otherwise default
  const targetValue = goal || 25;
  return Math.min(100, Math.round((numValue / targetValue) * 100));
}

function generateRealData(habit: Habit, period: TimePeriod, mode: DateRangeMode): DataPoint[] {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  for (let i = daysToGenerate - 1; i >= 0; i--) {
    ...
    const activityValue = getActivityForDate(habit.dailyLogs, date);
    const daily = convertToPercentage(activityValue, habit.mode, habit.goal);
...
export const RollingTrendChart: React.FC<RollingTrendChartProps> = ({ habit, period }) => {
  ...
  const data = useMemo(() => generateRealData(habit, period, mode), [habit, period, mode]);
